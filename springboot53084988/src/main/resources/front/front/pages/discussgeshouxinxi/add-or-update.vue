	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussgeshouxinxi`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussgeshouxinxi`, params);
				...
				//跨表计算
						let oet = {}
						if(this.ruleForm.id){
							await this.$api.update(`discussgeshouxinxi`, this.ruleForm);
						}else{
							oet = await this.$api.add(`discussgeshouxinxi`, this.ruleForm);
						}
						uni.setStorageSync('discussgeshouxinxiCleanType',true);
						let rs2 = await this.$api.list('discussgeshouxinxi',{page: 1,limit: 10000,refid: this.ruleForm.refid})
						let totalscore = 0
						for(let x in rs2.data.list){
							totalscore += Number(rs2.data.list[x].score)
						}
						let rs3 = await this.$api.info('geshouxinxi',this.ruleForm.refid)
						rs3.data.totalscore = Number((totalscore / rs2.data.list.length).toFixed(2))
						await this.$api.update('geshouxinxi',rs3.data)
