		...
		<view :style='{"alignContent":"flex-start","alignItems":"flex-start","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20241029/d192bff62bc241578876b935312772da.png) no-repeat center top / 100% auto,#f8f8f8","display":"flex","width":"100%","height":"auto"}'>
			...
			<view class="listBox recommend" :style='{"width":"100%","padding":"0 24rpx","margin":"40rpx auto","background":"none","order":"4"}'>
				<view class="title" :style='{"padding":"0 0 0 20rpx","margin":"0 0 40rpx","borderColor":"#23baf7","textAlign":"left","background":"none","borderWidth":"0 0 0 12rpx","width":"100%","position":"relative","borderStyle":"solid","fontWeight":"600"}'>
					<view :style='{"color":"#23baf7","fontSize":"48rpx"}'>歌曲信息推荐</view>
				</view>
				<!-- 样式1 -->
				<view class="list-box style1" :style='{"width":"100%","columnCount":"2","columnGap":"30rpx","height":"auto"}'>
					<view @tap="onDetailTap('gequxinxi',product.id)" v-for="(product,index) in gequxinxilist" :key="index" class="list-item" :style='{"padding":"0 0 20rpx","margin":"0 0 40rpx","borderRadius":"20rpx","flexWrap":"wrap","background":"#fff","display":"flex","width":"auto","height":"auto"}'>
						<view :style='{"padding":"0 20rpx","overflow":"hidden","whiteSpace":"nowrap","color":"#000","width":"100%","lineHeight":"1.8","fontSize":"28rpx","textOverflow":"ellipsis"}' class="list-item-title ">歌名:{{product.songname}}</view>
						<view :style='{"padding":"0 20rpx","overflow":"hidden","whiteSpace":"nowrap","color":"#000","width":"100%","lineHeight":"1.8","fontSize":"28rpx","textOverflow":"ellipsis"}' class="list-item-title ">歌手:{{product.singer}}</view>
						<view :style='{"padding":"0 20rpx","overflow":"hidden","whiteSpace":"nowrap","color":"#000","width":"100%","lineHeight":"1.8","fontSize":"28rpx","textOverflow":"ellipsis"}' class="list-item-title ">{{product.gequfenlei}}</view>
						<image :style='{"padding":"0","margin":"0","objectFit":"cover","borderRadius":"20rpx 20rpx 0 0","display":"block","width":"100%","height":"240rpx","order":"-1"}' class="list-item-image" mode="aspectFill" v-if="product.fengmian.substring(0,4)=='http'" :src="product.fengmian"></image>
						<image :style='{"padding":"0","margin":"0","objectFit":"cover","borderRadius":"20rpx 20rpx 0 0","display":"block","width":"100%","height":"240rpx","order":"-1"}' class="list-item-image" mode="aspectFill" v-else :src="product.fengmian?baseUrl+product.fengmian.split(',')[0]:''"></image>
						<view :style='{"color":"#51ca68","padding":"0 20rpx"}'>
						...
						<view :style='{"color":"#caa851","padding":"0 20rpx"}'>
						...
						<view :style='{"color":"#ca7651","padding":"0 20rpx"}'>
			...
			<view class="listBox recommend" :style='{"width":"100%","padding":"0 24rpx","margin":"40rpx auto","background":"none","order":"4"}'>
				<view class="title" :style='{"padding":"0 0 0 20rpx","margin":"0 0 40rpx","borderColor":"#23baf7","textAlign":"left","background":"none","borderWidth":"0 0 0 12rpx","width":"100%","position":"relative","borderStyle":"solid","fontWeight":"600"}'>
					<view :style='{"color":"#23baf7","fontSize":"48rpx"}'>歌单信息推荐</view>
				</view>
				<!-- 样式1 -->
				<view class="list-box style1" :style='{"width":"100%","columnCount":"2","columnGap":"30rpx","height":"auto"}'>
					<view @tap="onDetailTap('gedanxinxi',product.id)" v-for="(product,index) in gedanxinxilist" :key="index" class="list-item" :style='{"padding":"0 0 20rpx","margin":"0 0 40rpx","borderRadius":"20rpx","flexWrap":"wrap","background":"#fff","display":"flex","width":"auto","height":"auto"}'>
						<view :style='{"padding":"0 20rpx","overflow":"hidden","whiteSpace":"nowrap","color":"#000","width":"100%","lineHeight":"1.8","fontSize":"28rpx","textOverflow":"ellipsis"}' class="list-item-title ">{{product.mingcheng}}</view>
						<image :style='{"padding":"0","margin":"0","objectFit":"cover","borderRadius":"20rpx 20rpx 0 0","display":"block","width":"100%","height":"240rpx","order":"-1"}' class="list-item-image" mode="aspectFill" v-if="product.fengmian.substring(0,4)=='http'" :src="product.fengmian"></image>
						<image :style='{"padding":"0","margin":"0","objectFit":"cover","borderRadius":"20rpx 20rpx 0 0","display":"block","width":"100%","height":"240rpx","order":"-1"}' class="list-item-image" mode="aspectFill" v-else :src="product.fengmian?baseUrl+product.fengmian.split(',')[0]:''"></image>
						<view :style='{"padding":"0 20rpx","overflow":"hidden","whiteSpace":"nowrap","color":"#000","width":"100%","lineHeight":"1.8","fontSize":"28rpx","textOverflow":"ellipsis"}' class="list-item-title ">{{product.singer}}</view>
						<view :style='{"padding":"0 20rpx","overflow":"hidden","whiteSpace":"nowrap","color":"#000","width":"100%","lineHeight":"1.8","fontSize":"28rpx","textOverflow":"ellipsis"}' class="list-item-title ">{{product.chuangjianriqi}}</view>
						<view :style='{"color":"#51ca68","padding":"0 20rpx"}'>
							<text class="icon iconfont icon-shijian21" :style='{"margin":"0 4rpx 0 0","lineHeight":"1.5","fontSize":"inherit","color":"inherit"}'></text>
							<text :style='{"color":"inherit","lineHeight":"1.5","fontSize":"inherit"}'>{{product.addtime.split(' ')[0].replace(/\-/g,'-')}}</text>
						</view>
			...
			<!-- 商品列表 -->
			<!-- 新闻资讯 -->
			<view class="listBox news" :style='{"padding":"24rpx","margin":"40rpx auto","borderRadius":"20rpx","flexWrap":"wrap","background":"#fff","display":"flex","width":"calc(100% - 48rpx)","position":"relative","height":"auto","order":"2"}'>
				<view class="title" :style='{"padding":"0 0 0 20rpx","margin":"0 0 40rpx","borderColor":"#23baf7","textAlign":"left","background":"none","borderWidth":"0 0 0 12rpx","width":"100%","position":"relative","borderStyle":"solid","fontWeight":"600"}'>
					<view :style='{"color":"#23baf7","fontSize":"48rpx"}'>公告信息</view>
					<view :style='{"padding":"0 40rpx","alignItems":"center","top":"0px","borderRadius":"12rpx","background":"#23baf7","display":"flex","lineHeight":"72rpx","position":"absolute","right":"0px","justifyContent":"center","zIndex":"999"}' @tap="onPageTap('news')">
				...
				<!-- 样式2 -->
				<view class="list-box style2" :style='{"width":"100%","padding":"0","height":"auto"}'>
					<view class="tabView" :style='{"width":"100%","flexWrap":"wrap","background":"#eee","justifyContent":"center","display":"none"}'>
						<view class="tab" :class="newsIndex2==index?'tabActive':''" v-for="(item,index) in newsCateList2" :key="index" @click="newsTabClick2(index)">
							<text class="icon iconfont icon-zhankai19" :style='{"margin":"0 4rpx 0 0","lineHeight":"88rpx","fontSize":"28rpx","color":"inherit","display":"none"}'></text>
							<text :style='{"color":"inherit","lineHeight":"88rpx","fontSize":"32rpx"}'>{{item.typename}}</text>
						</view>
					</view>
					<view :style='{"width":"100%"}'>
						<view @tap="onNewsDetailTap(item.id)" v-for="(item,index) in news" :key="index" class="list-item" :style='{"padding":"20rpx","borderColor":"#13b400","margin":"0 0 10rpx","overflow":"hidden","borderWidth":"0 0 0px 0","background":"#f6f8ff","display":"block","width":"100%","borderStyle":"dashed"}'>
							<view :style='{"alignItems":"center","color":"#fff","borderRadius":"20rpx","background":"#23baf7","flexDirection":"column","display":"flex","width":"176rpx","fontSize":"28rpx","float":"left","justifyContent":"center"}'>
								<view :style='{"fontSize":"44rpx","lineHeight":"1.5","color":"#fff","fontWeight":"600"}'>{{item.addtime.split(" ")[0].split("-")[2]}}</view>
								<view :style='{"color":"#fff","fontSize":"30rpx","lineHeight":"1.5"}'>{{item.addtime.split(" ")[0].split("-")[0]}}-{{item.addtime.split(" ")[0].split("-")[1]}}</view>
							</view>
							<view class="list-item-body" :style='{"width":"calc(100% - 200rpx)","float":"right","padding":"0 0 0 20rpx"}'>
								<view :style='{"padding":"0","margin":"0","overflow":"hidden","whiteSpace":"nowrap","color":"#000","width":"100%","lineHeight":"48rpx","fontSize":"30rpx","textOverflow":"ellipsis"}' class="title">{{item.title}}</view>
								<view :style='{"padding":"0","overflow":"hidden","whiteSpace":"nowrap","color":"#666","width":"100%","lineHeight":"1.5","fontSize":"28rpx","textOverflow":"ellipsis"}' class="text">{{item.introduction}}</view>
	...
	export default {
		...
		methods: {
			...
			async getRecommendList(){
				...
				if(uni.getStorageSync("appUserid")) {
					res = await this.$api.recommend2('gedanxinxi', params);
				} else {
					res = await this.$api.recommend('gedanxinxi', params);
				}
				this.gedanxinxilist = res.data.list
