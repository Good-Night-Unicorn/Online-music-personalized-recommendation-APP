		...
		<view class="container" :style='{"minHeight":"100vh","width":"100%","padding":"0","position":"relative","background":"#f8f8f8","height":"auto"}'>
			<view :style='{"alignContent":"flex-start","padding":"24rpx","margin":"0 auto","flexWrap":"wrap","background":"none","display":"flex","width":"100%","position":"relative","height":"auto","zIndex":"9"}' class="detail-content">
				...
				<view class="bottom-content bg-white tabbar border shop" :style='{"padding":"0","margin":"40rpx auto","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto","order":"3"}'>
					...
					<button :style='{"border":"0","padding":"0 20rpx","boxShadow":"none","margin":"0 10rpx 10rpx 0","color":"rgb(255, 255, 255)","borderRadius":"0","background":"#ae4341","width":"auto","fontSize":"28rpx","lineHeight":"80rpx","height":"80rpx","order":"8"}' v-if="userid&&isAuth('discussgequxinxi','私聊')" @tap="chatClick">联系TA</button>
					<button :style='{"border":"0","padding":"0 20rpx","boxShadow":"none","margin":"0 10rpx 10rpx 0","color":"rgb(255, 255, 255)","borderRadius":"0","background":"#ae4341","width":"auto","fontSize":"28rpx","lineHeight":"80rpx","height":"80rpx","order":"8"}' v-if="!userid&&isAuthFront('discussgequxinxi','私聊')" @tap="chatClick">联系TA</button>
...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussgequxinxiCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussgequxinxiCleanType')
				this.mescroll.num = 1
			...
			if(crossCleanType) {
				...
				res = await this.$api.info('discussgequxinxi', this.id);
		...
		methods: {
			...
			async comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					await this.$api.update('discussdiscussgequxinxi',row)
					this.$utils.msg('点赞成功');
				}else {
					row.thumbsupnum--
					let arr = String(row.tuserids).split(',')
			...
			delClick(id){
				...
				uni.showModal({
					...
					async success(res) {
						if(res.confirm){
							await that.$api.del('discussdiscussgequxinxi',JSON.stringify([id]))
							that.$utils.msg('删除成功')
							that.detail.discussnum--
							await that.$api.update('discussgequxinxi',that.detail)
							setTimeout(()=>{
