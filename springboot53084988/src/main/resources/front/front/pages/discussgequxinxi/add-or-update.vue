			...
			async onSubmitTap() {
				...
				//跨表计算
						...
						if(this.ruleForm.id){
							...
							oet = await this.$api.add(`discussgequxinxi`, this.ruleForm);
						}
						uni.setStorageSync('discussgequxinxiCleanType',true);
						let rs2 = await this.$api.list('discussgequxinxi',{page: 1,limit: 10000,refid: this.ruleForm.refid})
						let totalscore = 0
						for(let x in rs2.data.list){
							totalscore += Number(rs2.data.list[x].score)
						}
						let rs3 = await this.$api.info('gequxinxi',this.ruleForm.refid)
						rs3.data.totalscore = Number((totalscore / rs2.data.list.length).toFixed(2))
						await this.$api.update('gequxinxi',rs3.data)
