<template>
	<div class="main-content" :style='{"width":"100%","padding":"20px 20px 20px 30px","fontSize":"15px"}'>
			...
			<el-form class="center-form-pv" :style='{"border":"20px solid #fff","margin":"0 0px 0px","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex"}' >
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">歌名</label>
						<el-input v-model="searchForm.songname" placeholder="歌名" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">歌手</label>
						<el-input v-model="searchForm.singer" placeholder="歌手" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">专辑</label>
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"0px 0","flexWrap":"wrap","background":"none","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('gequxinxi','歌曲分类统计')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#939393","height":"34px"}'></span>
						歌曲分类统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('gequxinxi','点击量统计')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#939393","height":"34px"}'></span>
						点击量统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('gequxinxi','收藏量统计')" type="success" @click="chartDialog3()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#939393","height":"34px"}'></span>
						收藏量统计
					</el-button>
				</el-row>
		...
		<el-dialog
			...
			width="800">
			<div id="gequfenleiChart1" style="width:100%;height:600px;"></div>
		...
		<el-dialog
			...
			width="800">
			<div id="clicknumChart2" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
	...
	export default {
		...
		methods: {
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					var gequfenleiChart1 = echarts.init(document.getElementById("gequfenleiChart1"),'macarons');
					this.$http({
						url: "gequxinxi/group/gequfenlei",
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							for(let i=0;i<res.length;i++){
								...
								xAxis.push(res[i].gequfenlei);
								yAxis.push(parseFloat((res[i].total)));
								pArray.push({
									value: parseFloat((res[i].total)),
									name: res[i].gequfenlei
								})
							}
							var option = {};
							let titleObj = this.pie.title
							titleObj.text = '歌曲分类统计'
							...
							// 使用刚指定的配置项和数据显示图表。
							gequfenleiChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								gequfenleiChart1.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			chartDialog2() {
				...
				this.$nextTick(()=>{
					...
					var clicknumChart2 = echarts.init(document.getElementById("clicknumChart2"),'macarons');
					this.$http({
						url: `gequxinxi/value/gequfenlei/clicknum`,
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							// 使用刚指定的配置项和数据显示图表。
							clicknumChart2.setOption(option);
							//根据窗口的大小变动图表
							window.onresize = function() {
								clicknumChart2.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			chartDialog3() {
				...
				this.$nextTick(()=>{
					...
					this.$http({
						url: `gequxinxi/value/gequfenlei/storeupnum`,
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
